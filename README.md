...
The validation runs in one session over all trained clustering models from the given source path. A 3 level directory structure
is required, from the main source directory down to different metric results (Ex.: "m_1") down to different initialization results
(Ex.: "init_1"). Additionally the file/dataframe object for the vectors of all training instances needs to be in the upper source
directory, named "tweets_training_vecs.vec". If a change to the filename is required, this must be done in the source code of 
"cluster_eval.py". 
...
```
python run_evaluation.py --src resources/results/tweets_training_vecs600
