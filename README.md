...
The validation runs in one session over all trained clustering models from the given source path. 

Result of the session are figures of the class vs cluster validation, saved in the respective directories of the models.
```
...
A 3 level directory structure is required, from the main source directory down to different metric results (Ex.: "m_1") down to different initialization results (Ex.: "init_1"). Additionally the file/dataframe object for the vectors of all training instances needs to be in the upper source directory. If a change to the filename is required, this must be done in the source code of `cluster_eval.py`. 

An example of the needed directory structure (the values for m and the init strategies depend on the cluster runs you executed):
```
